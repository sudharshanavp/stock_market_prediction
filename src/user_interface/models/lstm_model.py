...
class LongShortTermMemory:
    ...
    def predict_values(self, model_path: str):
        ...
        last_data = self.input_df["Close"].tail(1).to_numpy()
        last_data = self.scalar_object.transform(last_data.reshape(1, -1))
        scaled_forecast = model.predict(last_data)
        unscaled_forecast = self.scalar_object.inverse_transform(
